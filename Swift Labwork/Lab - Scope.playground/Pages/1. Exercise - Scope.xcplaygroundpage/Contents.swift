...
//print("The value of foo is \(foo)") foo is not declared outside of the loop
...
//x is a global scope and foo is a local scope.
...
func greeting(greeting: String?, name: String) {
    if let greetingCheck = greeting {
        print("\(greetingCheck), \(name).")
    }
    else {
        print("Hello \(name)")
    }
