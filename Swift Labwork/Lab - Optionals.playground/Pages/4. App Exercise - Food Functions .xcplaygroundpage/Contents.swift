...
func MealType(_ Meal: String) -> Meal? {
    if let recordedMeal = meals[Meal] {
        return recordedMeal
    }
    else {
        return nil
 ...
 */
func checkMeals(_ arrayMeal: [String: Meal]) -> [String: Any]? {

    var newArrMeals: [String: Any] = [:]
    let result: [String: Any] = [:]

    if !arrayMeal.isEmpty {
        for item in arrayMeal {
            var mealLine: String = ""
            for food in item.value.food {
                mealLine += "\(food), "
            }

            mealLine += "\(item.value.calories) calories"

            newArrMeals[item.key] = mealLine
...
print(checkMeals(meals) ?? "Nothing is here")
